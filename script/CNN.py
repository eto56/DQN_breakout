...
class agent:
    ...
    def update(self):
        ...
        #actions = torch.from_numpy(self.data.action).long().to(self.device)
        #rewards = torch.from_numpy(self.data.reward).float().to(self.device)
        #dones = torch.from_numpy(self.data.done.astype(np.float32)).to(self.device)
        actions = self.data.action  # (B,)
        rewards = self.data.reward  # (B,)
        dones = self.data.done  # (B,)

        q_c = self.action_network(states)
        q = q_c[torch.arange(self.batch_size), actions]  # (B,)
